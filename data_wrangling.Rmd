...
colSums(is.na(edu.df)) # instateTuition (40), outstateTuition (40), grantAid (18), loanAid (127)
...
table(edu.df$hbcu) # 2612 No, 86 Yes
table(edu.df$tribal) # 2685 No, 13 Yes
...
table(edu.df$open) # 1870 No, 697 Yes, 131 Not applicable
# replace 'N/A' values in the 'open' column with 'No' values
edu.df$open[edu.df$open=="Not applicable"] <- "No"
table(edu.df$open) # 2001 No, 697 Yes

table(edu.df$landGrant) # 2614 Not a Land Grant, 84 Land Grant
...
table(edu.df$associates) # 1189 No, 1509 Yes
table(edu.df$allDistance) # 2659 No, 39 Yes
table(edu.df$partDistance) # 2010 Yes, 688 No
table(edu.df$studyAbroad) # 1582 Yes, 1116 No
table(edu.df$weekend) # 1366 No, 1332 Yes
table(edu.df$remedial) # 1874 Yes, 824 No
table(edu.df$counseling) # 2651 Yes, 47 No

table(edu.df$employment) # 2297 Yes, 401 No
table(edu.df$placement) # 2139 Yes, 559 No
...
table(edu.df$placeEmploy) # 2386 Yes, 312 No

table(edu.df$dayCare) # 2225 No, 473 Yes

table(edu.df$tuitionGuarantee) # 2529 No, 169 Yes
table(edu.df$tuitionPrepaid) # 2438 No, 260 Yes
table(edu.df$tuitionPlan) # 2257 Yes, 441 No
...
table(edu.df$tuitionOptions) # 2297 Yes, 401 No

table(edu.df$oncampusHousing) # 1763 Yes, 935 No

table(edu.df$mealPlan) # 1620 Yes (2 levels), 1078 No
...
table(edu.df$mealPlan) # 1620 Yes, 1078 No

# table(edu.df$instateTuition) # numerical, 40 missing values
# examine the average and spread of in-state tuition
mean(edu.df$instateTuition, na.rm=TRUE) # 18117
quantile(edu.df$instateTuition, na.rm=TRUE) # 0, 8732, 15300, 25868, 55944
# replace N/A values in the 'instateTuition' column with the mean
edu.df$instateTuition[is.na(edu.df$instateTuition)] <- 18117
colSums(is.na(edu.df))

# table(edu.df$outstateTuition) # numerical, 40 missing values
...
# there is a 90% correlation between out-of-state and in-state tuition
...
# table(edu.df$grantAid) # numerical, 18 missing values
...
# there is a correlation of 82% between grant aid and in-state tuition
...
# table(edu.df$loanAid) # numerical, 127 missing values
...
mean(edu.df$loanAid, na.rm=TRUE) # 7436
quantile(edu.df$loanAid, na.rm=TRUE) # 1060 6631 7306 8280 14400
# replace N/A values in the 'loanAid' column with the mean
edu.df$loanAid[is.na(edu.df$loanAid)] <- 7436
